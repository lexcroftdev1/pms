...
if ((isset($_GET['PurchaseOrderId'])) && ($_GET['PurchaseOrderId'] != ""))
{
	if((int)$UserGroup === 1)
	{
		$deleteSQL = sprintf("Update purchaseorder SET PurchaseOrderStatusId='30', Comments='Rejected by $clientname on behalf of client. Client Id is $LoggedInClientId', TimeStampWhenActed=now() WHERE PurchaseOrderId=%s",
		common::GetSQLValueString($_GET['PurchaseOrderId'], "int"));
	}
	else
	{
		$deleteSQL = sprintf("Update purchaseorder po inner join project p on po.ProjectId=p.ProjectId inner join client c on c.ClientId=p.ClientId SET PurchaseOrderStatusId='30', Comments='Rejected by $clientname. Client Id is $LoggedInClientId', po.TimeStampWhenActed=now() WHERE po.PurchaseOrderId=%s and c.ClientId =$LoggedInClientId",
						   common::GetSQLValueString($_GET['PurchaseOrderId'], "int"));
...
$deleteSQL = sprintf("Update project SET ProjectStatusId='10' WHERE ProjectId=%s",
	...
	$m = new POMail($_GET['PurchaseOrderId'],false);
